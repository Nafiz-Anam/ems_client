...
const Sidebar = () => {
  ...
  return (
    <>
      ...
      <aside
        className={` w-56 bg-primary h-screen lg:sticky absolute top-0 overflow-y-auto scrollbar-hide  flex-col  ${isLargeOpen ? "lg:flex" : "lg:hidden"} ${isSmallOpen ? "flex z-[999]" : "hidden"}`} >
        <div className="sticky top-0 px-2 z-[50]">
          <div className="flex flex-col items-center justify-center h-20">
            <div className="flex gap-2 mr-6">
              <img src={logo} alt="" />
              <h2 className='pt-3 text-2xl font-bold text-center text-white '>Xpert</h2>
            </div>
          </div>
          <ul className='mt-6'>
...
const Items = ({ data }) => {
  ...
  return (
    <li
      className="pb-2 rounded-r pt-3 text-sm leading-3 tracking-normal text-[#A1A1A1]  focus:outline-none ">
      <h1 className={`pb-4 font-inter text-md font-medium text-[#A1A1A1] `}>
        {data?.title}
      </h1>
      ...
      <div
        ...
        <div onClick={() => {
          ...
          <div className="flex items-center cursor-pointer">
            ...
            <p className={`pl-0 font-inter  font-semibold group-hover:text-white focus:text-white active:text-white text-sm  ${isActive && dropdown === data?.name && "text-white"}`}> {data?.name}  </p>
          </div>
          <div>
            {dropdown === data?.name ? (
              <div
