...
import { useRecoverAccountMutation } from "../../redux/features/auth/authApi";
import toast from "react-hot-toast";

const RecoverAccount = () => {
    const navigate = useNavigate();
    const { getUserDetails } = rememberMe();
    const authInfo = getUserDetails("autInfo")
        ? getUserDetails("autInfo")
        : null;

    const [recoverAccount, { error, isError, isSuccess, isLoading }] =
        useRecoverAccountMutation();

    const handleRecover = (e) => {
        e.preventDefault();
        recoverAccount({ email: e.target.Username.value });
    };
