...
const RecoverPassword = () => {
  ...
  const [resetPassword, { error, isLoading, isSuccess, isError }] = useResetPasswordMutation()

  const handleChangePassword = (e) => {
    e.preventDefault();
    const np = e.target.np.value
    const cp = e.target.cp.value
    if (np !== cp) {
      toast.error('Password not matched', { position: 'top-right', })
    } else { resetPassword({ password: np }) }
  }
  ...
  useEffect(() => {
    isError ?
      ...
      : isSuccess ?
        (toast.success('success', { position: 'top-right', }), navigate('/login'))
        : null
          ...
          <div className="px-4  pt-[4.063rem] pb-[6rem] sm:px-[3.125rem]">
            <div className="space-y-5">
              <div className="flex flex-col gap-2">
                <label
                  className="inter-500 text-base leading-[1.3rem] text-secondary"
                >
                ...
                <input
                  required
                  type="text"
                  className="poppins-400 rounded border-[0.031rem] border-[#fff] bg-transparent px-4 py-2.5 text-lg leading-[1.463rem] text-secondary"
              ...
              <div>
                <button
                  disabled={isLoading}
                  className={` w-full h-10 flex items-center justify-center  p-2.5 text-base font-bold leading-[1.3rem] t text-[#222222] ${isLoading ? " bg-gray-400 " : "bg-[#D3AC46]"}`}>
                  {isLoading ? <span className="loading loading-bars loading-sm"></span> : "Submit"}
                </button>
              </div>
            </div>
          </div>
